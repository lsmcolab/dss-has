...
//Reference website: https://stackoverflow.com/questions/1768197/bounding-ellipse

//Based on the algorithm for Minimum Volume Enclosing Ellipse
//https://uk.mathworks.com/matlabcentral/fileexchange/9542-minimum-volume-enclosing-ellipsoid
//Note that the code references a class called "Eigen" - a modified version of Jama's EigenvalueDecomposition class. 

//The test results are not right. Check the class and methods;
...
public class Ellipse {
   ...
   */
    public Ellipse(double[][] P, double tolerance){
        ...
        //u = (1/N) * ones(N,1) 
        //N=22 in the test case, u[i] = 1/22=0.045
 ...
 //       	System.out.println(u[i]); 
        ...
        // Khachiyan Algorithm - used to optimised the parameters of an enclosing ellipse
        while (err > tolerance){
...
//            System.out.println(new_u[j]+",  "+step_size);
...
//            System.out.println(err);
        ...
        centerX = (double) transpose(c)[0][0];
        centerY = (double) transpose(c)[0][1];
//        center = new PVector ((float) transpose(c)[0][0], (float) transpose(c)[0][1]);
        ...
        //A=inv(A);
        //[Ve,De]=eig(A);
        //eign is a double ArrayList, Eign() only take a matrix. So need to convert [inv(A)] into a matrix
        Eign eig = new Eign(inv(A));
        Matrix Ve1 = eig.getV(); //eigenvalues
        Matrix De1 = eig.getD(); //right eigenvectors
        
        //Convert Matrix Ve1 and De1 to double [][] ArrayList Ve and De
        double [][] Ve = Ve1.getArray();
        double [][] De = De1.getArray();
...
//		}
        ...
        for (int i=0; i<veig.length; i++){
            veig[i] = Ve[Ie][i];
//            System.out.println(veig[i]);
        }

        //thu=atan2(veig(2),veig(1)); The angle between the two orthogonal direction (r_x and r_y)
        thu = Math.atan2(veig[1], veig[0]);
//        System.out.println(veig[1]+","+veig[0]);

        if (thu>=0) {
        	ang = thu*180/Math.PI;
        } else {
        	ang = 180 + (thu*180/Math.PI);
        }
...
//**************************************************************************
  //** getCenter
  //**************************************************************************
  /** Returns the center point of the ellipse and other key features (modified)
   */
...
//**************************************************************************
   ...
   */
    public double[][] getBoundingCoordinates(int numPoints){
        ...
        for (int i=0; i<coords.length; i++){
            double x = coords[i][0] + centerX;
            double y = coords[i][1] + centerY;
 ...
 ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //Redefine basic mathematical methods using double[], not matrix
  //**************************************************************************
  //** reorderEigenVectors
  //**************************************************************************
